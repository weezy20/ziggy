...
// Hardcoded lean Zig app template
...
/**
 * Download and extract the lean Zig app template using fetch API
 */
export async function cloneTemplateRepository(
  ...
  console.log(colors.blue(`Downloading lean Zig app template...`));
  ...
  try {
    ...
    const extractedDir = join(tempDir, 'zig-app-template-main');
    if (existsSync(extractedDir)) {
      ...
      console.log(colors.green(`✓ Template extracted to ${targetPath}`));
    } else {
      throw new Error('Failed to find extracted template directory');
    }
    
    // Clean up temporary files
    console.log(colors.blue(`Cleaning up temporary files...`));
    if (existsSync(tempDir)) {
